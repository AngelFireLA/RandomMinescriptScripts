...
def path_find(start, goal, do_smooth=True, closest_if_fail=False):
    """
    ...
    Args:
        ...
        do_smooth: Whether to apply path smoothing (default True)
        closest_if_fail: If True, return path to closest reachable position 
                         when no complete path is found (default False)
    
    Returns:
        List of waypoints from start to goal (or closest reachable position)
    
    Raises:
        ValueError: If no path found and closest_if_fail is False
    """
